...
<div class="row">
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body">
                <form action="<?= base_url('/sms/master/teaching-schedule/update') ?>" method="POST">
                    <?= csrf_field(); ?>
                    <input type="hidden" name="teaching_schedule_id" value="<?= $row['teaching_schedule_id'] ?>">
                    <div class="mb-10">
                        <?php
                        $fe = $le = $msg = '';
                        ...
                        $opt = old('day') ? old('day') : $row['teaching_schedule_day'];
                        ?>
                        <label for="day" class="required form-label <?= $le ?>">Hari <?= $row['teaching_schedule_day'] ?></label>
                        <select class="form-select form-control-md" id="day" name="day">
                            <option value="">Pilih Hari</option>
                            <?php foreach ($days as $k => $v): ?>
                                <option value="<?= $k ?>" <?= $opt == $k ? 'selected' : '' ?>>
                                    <?= $v ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="mb-10">
                        ...
                        $opt = old('order') ? old('order') : $row['teaching_schedule_order'];
                        ?>
                        ...
                        <div class="row mb-5">
                            ...
                            <div class="col-md-6 fv-row">
                                ...
                                <input class="form-control <?= $fe ?>" name="time2" id="time2"
                                    value="<?= old('time2') ? old('time2') : $val[1] ?>" />
