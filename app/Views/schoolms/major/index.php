            ...
            <tbody>
                <?php foreach ($row as $k => $v): ?>
                    <tr>
                        ...
                        <td><?= $v['major_name'] ?></td>
                        <td><?= $v['major_description'] ?></td>
                        <td class="d-flex">
                            <div
                                class="d-flex flex-grow-1 justify-content-center form-check form-switch form-check-custom form-check-solid">
                                <input class="form-check-input" id="chx_<?= $v['major_id'] ?>" type="checkbox" value="<?= $v['major_status'] ?>" data-id="<?= $v['major_id'] ?>"
                                    <?= check_status($v['major_status']) ?> />
                            </div>
                        </td>
                        <td class="text-center">
                            <?= button_edit('/sms/master/major/edit/' . $v['major_id']) ?>
                            <?= button_delete('/sms/master/major/destroy/', $v['major_id']) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
